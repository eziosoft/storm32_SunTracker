...
public class MAVLinkCRC {
     ...
     */
    private final static byte MAVPROT_PACKET_START_V10 = (byte) 0xFE;

    /**
     * Packet start in MAVLink V0.9
     */
    private final static byte MAVPROT_PACKET_START_V09 = (byte) 0x55;

    /**
     * Len to add to payload for CRC computing
     */
    private final static int CRC_LEN = 5;

    /**
     * Use to initialize CRC before computing
     */
    private final static int X25_INIT_CRC = 0x0000ffff;

    /**
     * Use to validate CRC
     */
    private final static int X25_VALIDATE_CRC = 0x0000f0b8;
    ...
    public static int crc_calculate_decode(byte[] buffer, int dataLength) {
        int crc = crc_init();
        for (int i = 1; i <= dataLength + CRC_LEN; i++) {
     ...
     * @param buffer buffer containing the byte array to hash
     * @param length total length of the MAVLink message so CRC is calculate between 1 and length - 2
     * @return the accumulated checksum
     */
